    ...
    for pdb in pdb_list:
        ...
        for sheet in beta_sheets_pre:
            for strand in sheet.strands:
                ...
                if seq is not None:
                    beta_sheets.append(f"model {strand.pdbID} and chain {strand.start_chain} and resi "
                                       f"{strand.start_resi}-{strand.end_resi}")
        for barrel in beta_barrels_pre:
            for strand in barrel.strands:
                seq = strand.get_strand_sequence(pdb_seq)[0]
                if seq is not None:
                    beta_barrels.append(f"model {strand.pdbID} and chain {strand.start_chain} and resi "
                                        f"{strand.start_resi}-{strand.end_resi}")

    write_file(beta_sheets, beta_barrels, pdb_list)
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def __init__(self, pdb_sequence, strands):
        ...
        if len(self.strands) == 1:
            ...
            for n in range(len(strand_sequence_idx_list)):
                ...
                strand_sequence_idx_list_not_n = [strand_sequence_idx_list[i] for i in
                                                  range(len(strand_sequence_idx_list))
                                                  if i != n]
...
def write_file(beta_sheets, beta_barrels, pdb_list_old):
    pdb_str = str()
    for pdb in pdb_list_old:
        pdb_str += pdb + ','
    with open("sheets.txt", "w") as s:
        s.write(pdb_str[:-1]+'\n')
        for strand_seq in beta_sheets:
            s.write(str(strand_seq) + '\n')

    with open("barrels.txt", 'w') as b:
        for strand_seq in beta_barrels:
            b.write(str(strand_seq) + '\n')
# ----------------------------------------------------------------------------------------------------------------------
